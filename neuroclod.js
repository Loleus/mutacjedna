...
const walls = [
    ...
    // ══ Przeszkody wewnętrzne - układ "zygzak" ══
    // Górna pozioma bariera (blokuje bezpośrednią drogę)
    { 
        x: Math.round(W * 0.25), 
        y: Math.round(H * 0.22), 
        w: Math.round(W * 0.55), 
    ...
    // Pionowa ściana przy górnej barierze (prawa strona)
    { 
        x: Math.round(W * 0.70), 
        y: Math.round(H * 0.22), 
        w: WALL_THICK, 
    ...
    // Pionowa ściana przy środkowej barierze (lewa strona)
    { 
        x: Math.round(W * 0.25), 
        y: Math.round(H * 0.48), 
        w: WALL_THICK, 
    ...
    // Pionowa ściana przy dolnej barierze
    { 
        x: Math.round(W * 0.55), 
        y: Math.round(H * 0.55), 
        w: WALL_THICK, 
    ...
    // Dodatkowa mała przeszkoda w centrum
    { 
        x: Math.round(W * 0.42), 
        y: Math.round(H * 0.32), 
        w: WALL_THICK, 
...
class Agent {
    ...
    computeFitness() {
        // ═══ FUNKCJA FITNESS ═══
        // Cel: maksymalizować postęp w kierunku celu
        
        if (this.reached) {
            // OSIĄGNĄŁ CEL: bazowy wysoki fitness + bonus za szybkość
            const speedBonus = (1 - this.step / STEP_LIMIT) * 3;
            this.fitness = 10.0 + speedBonus;
            return this.fitness;
        }

        // Postęp = jak blisko dotarł względem dystansu start→goal
        // Używamy minDist (najlepsza osiągnięta pozycja)
        const progress = 1 - (this.minDist / START_TO_GOAL_DIST);
        
        // Skaluj postęp do 0-7 punktów
        const progressScore = Math.max(0, progress) * 7;

        // Mały bonus za przeżycie (zachęca do eksploracji)
        const aliveBonus = this.dead ? 0 : 0.3;

        // Bonus za długie przetrwanie (eksploracja)
        const exploreBonus = (this.step / STEP_LIMIT) * 0.2;

        this.fitness = progressScore + aliveBonus + exploreBonus;
        this.fitness = Math.min(this.fitness, 9.99);  // cap poniżej "reached"
